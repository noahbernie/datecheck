...
function App() {
  ...
  const PricingTier = ({
    ...
    <div
      className={`relative p-8 rounded-2xl backdrop-blur-lg transition-all duration-500 ${
        highlighted
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const filterResponse = await fetch('http://127.0.0.1:5000/api/filter-results', {
        ...
        body: JSON.stringify({ results: uploadData.results }),
      });
      ...
      const prepareDisplayResponse = await fetch(
        'http://127.0.0.1:5000/api/prepare-display-data',
        {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            instagram: filterData.instagram || [],
            linkedin: filterData.linkedin || [],
            twitter: filterData.twitter || [],
            facebook: filterData.facebook || [],
            others: filterData.others || [],
          }),
        }
      );
      ...
      setUploadResults(preparedDisplayData.display_data);
      setShowAuth(true);
    } catch (error: any) {
    ...
    {!showAuth && !showPaywall && !showInsights ? (
      <div className="container mx-auto px-4 py-20">
        ...
        <div className="max-w-2xl mx-auto">
          <div
            className={`relative rounded-xl p-8 bg-white/10 backdrop-blur-lg border-2 border-dashed ${
              dragActive ? 'border-pink-300 bg-white/20' : 'border-white/50'
